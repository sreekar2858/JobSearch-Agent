...
async def _run_job_search(search_id: str, keywords: str, locations: List[str], 
                         job_type: str, experience_level: str, max_jobs: int):
...
async def parse_job_posting(request: JobParseRequest):
    """Parse job details from text or file content"""
    try:
...
async def _run_job_process(process_id: str, job_posting: Dict[str, Any], 
                          generate_cv: bool, generate_cover_letter: bool):
    ...
    try:
        ...
        # Save job metadata
        metadata_path = os.path.join(folder_name, f'{company}_{job_title}_metadata.json')
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(job_posting, f, indent=2)
        results["files"]["metadata"] = metadata_path.replace(os.path.normpath(output_dir), "output")
        ...
        if generate_cv:
            try:
                ...
                results["files"]["cv"] = cv_file_path.replace(os.path.normpath(output_dir), "output")
            except Exception as e:
        ...
        if generate_cover_letter:
            try:
                ...
                results["files"]["cover_letter"] = cl_file_path.replace(os.path.normpath(output_dir), "output")
            except Exception as e:
                results["cover_letter_error"] = str(e)
        
        # Write results file
        with open(os.path.join(output_dir, f"{process_id}.json"), 'w', encoding='utf-8') as f:
            json.dump(results, f, indent=2)
            
    except Exception as e:
        # Log the error
        with open(os.path.join(output_dir, f"{process_id}_error.txt"), 'w', encoding='utf-8') as f:
            f.write(str(e))
...
async def handle_ws_parse(websocket: WebSocket, message_data: dict):
    ...
    try:
        ...
        if not text:
            await manager.send_error(websocket, "No text provided for parsing")
            return
    ...
    try:
        ...
        if generate_cover_letter:
            ...
            try:
                ...
                await manager.send_progress(websocket, "Cover letter generated successfully")
            except Exception as e:
                await manager.send_progress(websocket, f"Error generating cover letter: {str(e)}")
                results["cover_letter_error"] = str(e)
        ...
        if "cv" in results:
            ...
            results["cv_file"] = cv_file_path.replace(os.path.normpath(output_dir), "output")
        ...
        if "cover_letter" in results:
            ...
            # Add file path to results
            results["cover_letter_file"] = cl_file_path.replace(os.path.normpath(output_dir), "output")
