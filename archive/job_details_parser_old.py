...
class JobParsr(BaseAgent):
    ...
    async def _run_async_impl(
        ...
        if event.error_message:
            logger.error(
                f"[{self.name}] Error in deciding parser: {event.error_message}"
            )
            return
        ...
        if "json" in parser_decision.lower():
            ...
            if event.error_message:
                logger.error(
                    f"[{self.name}] Error in JSON validation: {event.error_message}"
                )
                return

            logger.info(
                f"[{self.name}] JSON validation result: {event.content.parts[0].text}"
            )
            ...
            if "valid" in event.content.parts[0].text:
                ...
                if event.error_message:
                    logger.error(
                        f"[{self.name}] Error in job validation: {event.error_message}"
                    )
                    return
                logger.info(
                    f"[{self.name}] Job validation result: {event.content.parts[0].text}"
                )
            ...
            if event.error_message:
                logger.error(
                    f"[{self.name}] Error in deciding scraper: {event.error_message}"
                )
                return
...
decidingParser = LlmAgent(
    ...
    model=(
        ...
        if "gemini" in agent_config.get("decidingParser_model")
            ...
            model=agent_config["models"][agent_config.get("decidingParser_model")]
        )
    ),
...
validateJob = LlmAgent(
    ...
    model=(
        ...
        if "gemini" in agent_config.get("validateJob_model")
            ...
            model=agent_config["models"][agent_config.get("validateJob_model")]
        )
    ),
...
decidingScraper = LlmAgent(
    ...
    model=(
        ...
        if "gemini" in agent_config.get("decidingScraper_model")
            ...
            model=agent_config["models"][agent_config.get("decidingScraper_model")]
        )
    ),
